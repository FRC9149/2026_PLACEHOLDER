...
private final double NULl = NULL;
  ...
  climbHeights.put(0, NULL); 
  climbHeights.put(1, NULL);
  climbHeights.put(2, NULL);
...
public void moveToHeight(double targetHeight, boolean hold) {
  ...
  setSpeed(
    ...
    private boolean withinLimits(double speed) {
        ...
        if (height >= MAX_HEIGHT && speed > 0) return false;
        if (height <= MIN_HEIGHT && speed < 0) return false;
